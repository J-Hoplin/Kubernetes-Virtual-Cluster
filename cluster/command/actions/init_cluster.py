...
def setup_single_master_cluster(connection, cursor, master_nodes, worker_nodes):
    ...
    master_cmd = f"curl -sfL https://get.k3s.io | K3S_TOKEN='{token}' sh -"
    ...
    setup_worker_nodes(connection, cursor, master_ip, token, worker_nodes)
...
def setup_ha_master_cluster(connection, cursor, master_nodes, worker_nodes):
    ...
    token = "".join(random.choices(string.ascii_letters + string.digits, k=16))
    master_init_cmd = f"curl -sfL https://get.k3s.io | K3S_TOKEN='{token}' sh -s - server --cluster-init"
    ...
    for master_name in master_names[1:]:
        ...
        join_cmd = f"curl -sfL https://get.k3s.io | K3S_TOKEN='{token}' sh -s - server --server https://{first_master_ip}:6443"
    ...
    setup_worker_nodes(connection, cursor, first_master_ip, token, worker_nodes)

    return True


def setup_worker_nodes(connection, cursor, master_ip, token, worker_nodes):
    if not worker_nodes:
    ...
    for worker_name in worker_nodes.keys():
        join_cmd = f"curl -sfL https://get.k3s.io | K3S_TOKEN='{token}' K3S_URL='https://{master_ip}:6443' sh -"
