    ...
    try:
        for node_name, node_type in nodes:
            if node_type == "master":
                ...
                subprocess.run(["kubectl", "config", "delete-context", context_name],
                               stderr=subprocess.DEVNULL,
                               stdout=subprocess.DEVNULL)
                subprocess.run(["kubectl", "config", "delete-cluster", context_name],
                               stderr=subprocess.DEVNULL,
                               stdout=subprocess.DEVNULL)
                subprocess.run(["kubectl", "config", "delete-user", context_name],
                               stderr=subprocess.DEVNULL,
                               stdout=subprocess.DEVNULL)
        return True
